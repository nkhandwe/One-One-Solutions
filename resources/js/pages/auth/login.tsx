...
export default function Login({ status, canResetPassword, setting }: LoginProps) {
    ...
    const appName = setting?.app_name || 'ModernApp';
    const appLogo = setting?.logo_light;
    console.log(appLogo);
    const features = [
        {
        ...
        <>
            ...
            <div className={`relative min-h-screen overflow-hidden transition-colors duration-500 ${isDark ? 'dark' : ''}`}>
                ...
                <div ref={orbsRef} className="pointer-events-none fixed inset-0">
                    <div className="animate-float absolute top-1/4 left-1/4 h-64 w-64 rounded-full bg-gradient-to-r from-blue-400/20 to-cyan-400/20 blur-3xl" />
                    <div className="absolute top-3/4 right-1/4 h-96 w-96 animate-pulse rounded-full bg-gradient-to-r from-purple-400/20 to-pink-400/20 blur-3xl" />
                    <div
                        className="animate-float absolute bottom-1/4 left-1/3 h-80 w-80 rounded-full bg-gradient-to-r from-green-400/20 to-emerald-400/20 blur-3xl"
                        style={{ animationDelay: '2s' }}
                    />
                    <div
                        className="absolute top-1/2 right-1/3 h-72 w-72 animate-pulse rounded-full bg-gradient-to-r from-orange-400/20 to-red-400/20 blur-3xl"
                        ...
                        <div className="relative z-10 flex flex-1 flex-col justify-center px-12 xl:px-20">
                            ...
                            <div className="mb-12">
                                <div ref={logoRef} className="mb-8 flex items-center space-x-4">
                                    {appLogo ? (
                                        <img src={appLogo} alt={appName} className="h-20 w-100 rounded-2xl shadow-2xl border border-white bg-white" />
                                    ) : (
                            ...
                            {/* Features showcase - FIXED */}
                            <div ref={featuresRef} className="mb-12">
                                {/* Feature cards container with proper height */}
                                <div className="relative h-96 w-full">
                                    {features.map((feature, index) => (
                                        <div
                                            key={index}
                                            className={`feature-card absolute inset-0 transform transition-all duration-700 ease-in-out ${
                                                index === currentFeature ? 'translate-y-0 scale-100 opacity-100' : 'translate-y-8 scale-95 opacity-0'
                                            }`}
                                        >
                                            <div className="h-full rounded-3xl border border-gray-200/50 bg-white/60 p-8 shadow-2xl backdrop-blur-xl dark:border-gray-700/50 dark:bg-gray-800/60">
                                                <div
                                                    className={`mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br ${feature.color} text-white shadow-lg`}
                                                >
                                                    {feature.icon}
                                                </div>
